...
<task type="auto">
  ...
  <action>
**Step 1: Create service.ts with service role client**
...
**Note:** If service.ts already exists, executor will use Edit tool to update it. The key requirement is that createServiceClient export exists before validation.ts imports it.
  </action>
  <verify>
```bash
...
7. service.ts created with createServiceClient export, and validation.ts successfully imports it
...
- validation.ts successfully imports createServiceClient from service.ts
