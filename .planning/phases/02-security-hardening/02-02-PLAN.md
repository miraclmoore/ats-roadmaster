...
- Used in API routes BEFORE performing operations on user resources
  ...
  <verify>
```bash
# First verify service.ts exists and exports createServiceClient
...
7. service.ts dependency verified to exist and export createServiceClient BEFORE creating validation.ts
</verification>
...
- validation.ts successfully imports createServiceClient from service.ts (verified before file creation)
</success_criteria>
