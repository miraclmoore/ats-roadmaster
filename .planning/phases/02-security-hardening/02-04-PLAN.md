...
Output: All API routes secured with layered defense: validation first (fast path rejection), authentication second (identify user), rate limiting third (prevent per-user abuse), secondary validation fourth (prevent privilege escalation), Sentry throughout (observes errors).
</objective>
...
```bash
# Verify schema files from plan 02-02 exist before importing
ls web/app/api/jobs/start/schema.ts web/app/api/jobs/complete/schema.ts
```
...
4. Job endpoints use mutationLimiter (100/hour)
5. Regenerate key uses authLimiter (20/15min)
6. All routes validate input with Zod before operations
...
10. All success responses include X-RateLimit-* headers
</verification>
