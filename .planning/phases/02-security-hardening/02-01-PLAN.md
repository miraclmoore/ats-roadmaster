...
user_setup:
  - service: upstash
    why: "Redis-backed rate limiting for API protection"
    env_vars:
      - name: UPSTASH_REDIS_REST_URL
        source: "Upstash Console (https://console.upstash.com/redis) -> Create Database -> Copy REST URL from database details"
      - name: UPSTASH_REDIS_REST_TOKEN
        source: "Upstash Console -> Database Details -> Copy REST Token"
    dashboard_config:
      - task: "Create Upstash account (free tier available)"
        location: "https://console.upstash.com/login"
      - task: "Create new Redis database (select Global for multi-region support)"
