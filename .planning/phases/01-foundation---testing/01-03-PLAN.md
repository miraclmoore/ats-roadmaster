...
must_haves:
  truths:
    - "Developer runs npm run test:db and sees database trigger tests pass"
    ...
    - "Tests verify profit calculation trigger works correctly"
  artifacts:
    - path: "supabase/tests/001-database.test.sql"
      provides: "pgTAP tests for database triggers and constraints"
      min_lines: 50
...
<objective>
Establish pgTAP database testing infrastructure and verify Row Level Security policies and database triggers function correctly.

Purpose: Ensure data isolation between users and accurate profit calculations at database level
...
<task type="auto">
  ...
  <verify>
    Run `cd web && supabase db test --help` and see pgTAP test command help
...
<task type="auto">
  <name>Write database trigger and constraint tests</name>
  <files>
    supabase/tests/001-database.test.sql
  </files>
  <action>
    ...
    Header comment:
    ```sql
    -- Database Tests: Triggers, Constraints, and Calculated Fields
  ...
  <done>
    001-database.test.sql exists with 15+ tests verifying jobs and telemetry table structure, indexes, and constraints
  </done>
</task>
...
- Database structure tests verify required columns, indexes, and constraints exist
