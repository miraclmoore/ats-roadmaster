...
<task type="auto">
  ...
  <action>
    Add database test scripts to ROOT package.json (NOT web/package.json):
    ```json
    ...
    CRITICAL: Scripts added to ROOT package.json because supabase/ directory is at project root
  </action>
  <verify>
    Run `supabase --version` from project root and see Supabase CLI version
    Verify supabase/tests/ directory exists
    Check package.json (root) contains test:db script
  </verify>
  <done>
    Database test scripts added to root package.json, test directory created, documentation comments prepared
  </done>
</task>
...
<task type="auto">
  ...
  <verify>
    Run `npm run test:db` from project root and see database structure tests pass
...
<task type="auto">
  ...
  <verify>
    Run `npm run test:db` from project root and see all RLS tests pass
...
<verification>
Run `npm run test:db` from project root and see:
...
<success_criteria>
- Developer runs `npm run test:db` from project root and sees all database tests pass
...
- Test scripts located in root package.json where supabase/ directory is accessible
</success_criteria>
