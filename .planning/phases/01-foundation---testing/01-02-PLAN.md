...
must_haves:
  truths:
    ...
    - "Component tests verify critical UI components render correctly (Gauge, RouteAdvisorCard, LiveDashboard)"
  ...
  artifacts:
    ...
    - path: "web/__tests__/components/live-dashboard.test.tsx"
      ...
      min_lines: 80
    - path: "web/__tests__/components/gauge.test.tsx"
      provides: "Component render tests for Gauge widget"
      min_lines: 40
    - path: "web/__tests__/components/route-advisor-card.test.tsx"
      provides: "Component render tests for RouteAdvisorCard"
      min_lines: 40
  key_links:
    ...
    - from: "web/__tests__/components/gauge.test.tsx"
      to: "@/components/ui/gauge"
      via: "import statement"
      pattern: "import.*Gauge.*from.*@/components/ui/gauge"
---

<objective>
Establish Playwright E2E testing infrastructure and verify real-time telemetry flow works end-to-end, including component-level tests with WebSocket mocking and UI component render tests.

Purpose: Ensure critical user journey (viewing live telemetry) works across authentication, data fetching, and real-time updates, with component-level confidence that UI widgets render correctly
Output: Working E2E test suite that can run in CI pipeline plus component tests with mocked Supabase real-time and render tests for critical UI components
</objective>
...
# Existing UI components to test
@web/components/ui/gauge.tsx
@web/components/telemetry/route-advisor-card.tsx
</context>
...
<task type="auto">
  <name>Write component render tests for critical UI components</name>
  <files>
    web/__tests__/components/gauge.test.tsx
    web/__tests__/components/route-advisor-card.test.tsx
  </files>
  <action>
    Create component render tests for critical UI components using @testing-library/react following RESEARCH.md patterns.

    **File: web/__tests__/components/gauge.test.tsx**
    ...
    **Test 1: "renders with basic props"**
    - Render Gauge with value=50, max=100
    - Verify component renders without crashing
    - Check for gauge visual element in DOM

    **Test 2: "displays correct value"**
    - Render Gauge with value=75, max=100
    - Verify value text is displayed (75 or 75%)

    **Test 3: "handles zero value"**
    - Render Gauge with value=0
    - Verify renders without errors
    - Check for appropriate empty/zero state

    **Test 4: "handles max value"**
    - Render Gauge with value=100, max=100
    - Verify renders at full capacity
    - Check for visual indicator of full state

    **Test 5: "applies custom label"**
    - Render Gauge with label="Speed"
    - Verify label text is visible
    - Check accessibility attributes

    **File: web/__tests__/components/route-advisor-card.test.tsx**
    ...
    **Test 1: "renders with route data"**
    - Render RouteAdvisorCard with origin="Los Angeles", destination="San Francisco"
    - Verify both city names are displayed
    - Check for route information rendering

    **Test 2: "displays cargo information"**
    - Render with cargo="Electronics", weight=20000
    - Verify cargo details are shown
    - Check for weight formatting

    **Test 3: "shows income and distance"**
    - Render with income=5000, distance=380
    - Verify financial information displayed
    - Check for proper formatting ($5,000, 380 mi)

    **Test 4: "handles missing optional data"**
    - Render with only required props
    - Verify component doesn't crash
    - Check for graceful degradation

    **Test 5: "displays delivery deadline"**
    - Render with deadline timestamp
    - Verify deadline is shown
    - Check for time remaining calculation/display

    Use @testing-library/react best practices:
    - Query by accessible role/label (screen.getByRole, screen.getByLabelText)
    - Avoid implementation details (class names, internal state)
    - Test user-visible behavior, not implementation

    Reference existing production components at their actual file paths:
    - @/components/ui/gauge.tsx (confirmed exists from codebase scan)
    - @/components/telemetry/route-advisor-card.tsx (confirmed exists)

    IMPORTANT: Test PRODUCTION components, not stubs
    IMPORTANT: If components require complex props, use minimal viable data
  </action>
  <verify>
    Run `cd web && npm test gauge.test.tsx -- --run` and see all gauge tests pass
    Run `cd web && npm test route-advisor-card.test.tsx -- --run` and see all tests pass
    Run `cd web && npm test -- --run` and see all component render tests pass
  </verify>
  <done>
    Component render tests exist for Gauge and RouteAdvisorCard with @testing-library/react, verifying critical UI components render correctly with various props and states
  </done>
</task>
...
- Component render tests for Gauge and RouteAdvisorCard pass
...
- Component render tests verify Gauge and RouteAdvisorCard render correctly with various props
...
- Component tests use @testing-library/react best practices (accessible queries, user-visible behavior)
</success_criteria>
