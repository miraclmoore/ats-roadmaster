...
must_haves:
  truths:
    ...
    - "Developer runs npm test -- --coverage and sees coverage report with thresholds"
  artifacts:
    - path: "web/vitest.config.mts"
      provides: "Vitest configuration with jsdom environment and coverage reporting"
      min_lines: 20
...
<objective>
Establish Vitest unit testing infrastructure with coverage reporting and write comprehensive tests for profit/efficiency calculations and API routes.

Purpose: Enable safe refactoring of brownfield codebase by providing test coverage for critical business logic and API endpoints with measurable coverage metrics
Output: Working test suite with 60%+ coverage for calculation functions, API integration tests, and coverage reporting configured
</objective>
...
<task type="auto">
  <name>Install Vitest and configure test environment with coverage reporting</name>
  <files>
    web/package.json
    web/vitest.config.mts
    web/__tests__/setup.ts
  ...
  <action>
    ...
    ```bash
    cd web && npm install -D vitest @vitejs/plugin-react jsdom @testing-library/react @testing-library/dom vite-tsconfig-paths @vitest/coverage-v8
    ```
    ...
    CRITICAL: Coverage configuration uses v8 provider for accurate reporting
  </action>
  <verify>
    ...
    Run `cd web && npm run test:coverage` and see coverage report initialization
  </verify>
  <done>
    Vitest installed with @vitest/coverage-v8, config file created with jsdom environment, path alias resolution, and coverage reporting configured with 60% thresholds, test scripts available in package.json
  </done>
</task>
...
- Coverage thresholds configured (60% for branches, functions, lines, statements)
...
- Coverage reporting configured with v8 provider and 60% thresholds
- Developer runs `npm run test:coverage` and sees HTML coverage report with line-by-line coverage
</success_criteria>
